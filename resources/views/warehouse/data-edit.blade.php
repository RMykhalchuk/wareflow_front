...
@section('title', __('localization.warehouse.edit.title'))
...
@section('content')
    <x-layout.container id="edit-card" data-id="{{ $warehouse->id }}">
        @php
            $hasSchedule = $warehouse->schedule && count($warehouse->schedule) > 0;
        @endphp
            ...
            @include(
                ...
                [
                    'options' => [
                        [
                            'url' => '/warehouses',
                            'name' => __('localization.warehouse.edit.breadcrumb.warehouse'),
                        ],
                        [
                            'url' => '/warehouses/' . $warehouse->id,
                            'name' => __('localization.warehouse.edit.breadcrumb.view_warehouse', [
                                'name' => $warehouse->name,
                            ]),
                        ],
                        [
                            'name' => __('localization.warehouse.edit.breadcrumb.edit_warehouse'),
                        ],
                    ],
                ]
            )
        ...
        <x-slot:slot>
            <x-page-title :title="__('localization.warehouse.edit.title')" />
            ...
            <x-card.nested id="block_1">
                ...
                <x-slot:body>
                    <x-form.input-text
                        id="name"
                        name="name"
                        :label="__('localization.warehouse.create.block_1.name_label')"
                        :placeholder="__('localization.warehouse.create.block_1.name_placeholder')"
                    ...
                    <x-form.select
                        id="location"
                        name="location"
                        :label="__('localization.warehouse.create.block_1.location_label')"
                        :placeholder="__('localization.warehouse.create.block_1.location_placeholder')"
                        data-dictionary="location"
                        :data-id="$warehouse->location_id ?? null"
                    ></x-form.select>

                    <x-form.select
                        id="type"
                        name="type"
                        :label="__('localization.warehouse.create.block_1.type_label')"
                        :placeholder="__('localization.warehouse.create.block_1.type_placeholder')"
                        data-dictionary="warehouse_type"
                        :data-id="$warehouse->type_id ?? null"
                    ></x-form.select>

                    <x-form.select
                        id="erp-warehouse"
                        name="erp-warehouse"
                        :label="__('localization.warehouse.create.block_1.erp_warehouse_label')"
                        :placeholder="__('localization.warehouse.create.block_1.erp_warehouse_placeholder')"
                        data-dictionary="warehouses_erp"
                        multiple
                        data-id="{{ $warehouse->erpWarehouses->pluck('id')->toJson() }}"
                    ></x-form.select>
            ...
            <x-card.nested
                :bodyAttributes="[
                    'id' => 'working-data-body',
                    'style' => $hasSchedule ? 'display:block' : 'display: none;'
                ]"
            >
                <x-slot:header>
                ...
                <x-slot:body>
                    ...
                    <div class="col-12 px-0 col-lg-4 bg-light-secondary">
                        <div class="card bg-transparent h-100" id="condition-list">
                            ...
                            @php
                                $conditionNames = [
                                    'day_off' => 'localization.warehouse.conditions.day_off',
                                    'hospital' => 'localization.warehouse.conditions.hospital',
                                    'short_day' => 'localization.warehouse.conditions.short_day',
                                    'holiday' => 'localization.warehouse.conditions.holiday',
                                ];
                            @endphp
                            ...
                            @foreach ($warehouse->conditions as $condition)
                                ...
                                <div
                                    class="record border-bottom pb-1 row mx-0 mt-1"
                                    id="record_{{ $iteration }}"
                                >
                                    <div class="col-10">
                                        <div class="d-flex flex-wrap">
                                            <div class="w-100 mb-1">
                                                <span
                                                    class="f-15 fw-bold"
                                                    id="condition_{{ $iteration }}"
                                                >
                                                    {{ __($conditionNames[$condition->type->key] ?? '-') }}
                                                </span>
                                            </div>
                                            ...
                                            <div class="d-flex align-items-center mb-1">
                                                <img
                                                    ...
                                                    src="{{ asset('assets/icons/entity/warehouse/calendar-chosen-warehouse.svg') }}"
                                                    alt=""
                                                />
                                                ...
                                                @if ($condition->date_to)
                                                    <div>
                                                        ...
                                                        <span
                                                            class="f-15"
                                                            id="date_to_{{ $iteration }}"
                                                        >
                                                            {{ $condition->date_to }}
                                                        </span>
                                                    </div>
                                                @else
                                                    <span class="f-15" id="date_{{ $iteration }}">
                                                        {{ $condition->date_from }}
                                                    </span>
                                                @endif
                                            </div>
                                        ...
                                        @if ($condition->work_from && $condition->work_to)
                                            <div class="mb-1">
                                                {{ __('localization.warehouse.edit.render_conditions_work_day') }}:
                                                <span
                                                    class="hours f-15"
                                                    id="work_from_{{ $iteration }}"
                                                >
                                                    {{ $condition->work_from }}
                                                </span>
                                                -
                                        ...
                                        @if ($condition->break_from && $condition->break_to)
                                            <div>
                                                {{ __('localization.warehouse.edit.block_3_render_conditions_lunch') }}:
                                                <span
                                                    class="hours f-15"
                                                    id="break_from_{{ $iteration }}"
                                                >
                                                    {{ $condition->break_from }}
                                                </span>
                                                -
                                                <span
                                                    class="hours f-15"
                                                    id="break_to_{{ $iteration }}"
                                                >
                                                    {{ $condition->break_to }}
                                                </span>
                                            </div>
                                    ...
                                    <div class="col-2 row mx-0 align-self-start ps-0">
                                        <button
                                            class="btn edit-condition-back p-0 w-50"
                                            id="edit-condition-{{ $iteration }}"
                                            data-condition="{{ $iteration }}"
                                            data-name="{{ __($conditionNames[$condition->type->key] ?? '-') }}"
                                        >
                                            <img
                                                src="{{ asset('assets/icons/entity/warehouse/edit-btn.svg') }}"
                                                alt=""
                                            />
                                        </button>

                                        <button
                                            class="btn p-0 delete-condition-back w-50"
                                            data-condition="{{ $iteration }}"
                                            id="delete-condition-{{ $iteration }}"
                                        >
                                            <img
                                                src="{{ asset('assets/icons/entity/warehouse/delete-btn.svg') }}"
                                                alt=""
                                            />
                                        </button>
                                    </div>
    ...
    <x-cancel-modal
        ...
        title="{{ __('localization.warehouse.edit.cancel_modal.title') }}"
        content="{!! __('localization.warehouse.edit.cancel_modal.message') !!}"
        cancel-text="{{ __('localization.warehouse.edit.cancel_modal.cancel_button') }}"
        confirm-text="{{ __('localization.warehouse.edit.cancel_modal.confirm_button') }}"
    />
@endsection
...
@section('page-script')
    <script>
        ...
        let scheduleCheck = {!! json_encode($schedule) !!};
