        ...
        <x-slot:header>
            @include(
                ...
                [
                    'options' => [
                        ...
                        [
                            'name' => __('localization.sku_view_breadcrumb_view_product', [
                                'name' => $sku->name,
                            ]),
                        ],
                    ],
            ...
            <x-ui.header-actions>
                ...
                <x-ui.icon-link-button
                    href="{{ route('sku.edit', ['sku' => $sku->id]) }}"
                    ...
                    <div class="d-flex flex-grow-1 align-items-center justify-content-between">
                        <div class="d-flex align-items-center justify-content-center gap-1">
                            ...
                            <div class="d-flex justify-content-center flex-column gap-50">
                                ...
                                {{-- @php --}}
                                {{-- $barcodes = is_array($sku->barcodes) ? $sku->barcodes : json_decode($sku->barcodes, true); --}}
                                {{-- @endphp --}}

                                {{-- <p class="mb-0"> --}}
                                {{-- {{ implode(', ', array_map(fn($b) => $b['barcode'], $barcodes)) }} --}}
                                {{-- </p> --}}
                            </div>
                        </div>
                ...
                <x-slot:body>
                    ...
                    <x-ui.col-row-wrapper>
                        ...
                        <x-card.card-data-wrapper>
                            ...
                            <x-card.card-data-row :label="__('localization.sku_create_provider')">
                                {{ $sku?->providerCompany?->full_name }}
                            </x-card.card-data-row>

                            <x-card.card-data-row
                                :label="__('localization.sku_view_manufacturer')"
                            >
                                {{ $sku?->manufacturerCompany?->full_name }}
                            </x-card.card-data-row>

                            <x-card.card-data-row :label="__('localization.sku_view_brand')">
                                {{ $sku?->brandCompany?->full_name }}
                            </x-card.card-data-row>

                            <x-card.card-data-row
                                :label="__('localization.sku_view_manufacturer_country')"
                            >
                                <p class="m-0 fw-medium-c">
                                    {{ __("localization.sku_view_manufacturer_country_" . ($sku->manufacturer_country->key ?? "default")) ?? $sku->manufacturer_country->name }}
                                </p>
                            </x-card.card-data-row>

                            <x-card.card-data-row
                                :label="__('localization.sku_view_expiration_date')"
                                :value="collect($sku->expiration_date)->map(fn($d) => $d . ' ' . trans_choice('localization.sku_create_day_unit', $d))->implode(', ')"
                            />

                            <x-card.card-data-row
                                :label="__('localization.sku_view_batch_accounting')"
                            >
                                @if ($sku->is_batch_accounting)
                                    <span class="badge bg-success py-50 px-1">
                                        {{ __("localization.enabled") }}
                                    </span>
                                @else
                                    <span class="badge bg-danger py-50 px-1">
                                        {{ __("localization.disabled") }}
                                    </span>
                                @endif
                            </x-card.card-data-row>

                            <x-card.card-data-row
                                :label="__('localization.sku_view_weight_product')"
                            >
                                @if ($sku->is_weight)
                                    <span class="badge bg-success py-50 px-1">
                                        {{ __("localization.enabled") }}
                        ...
                        <x-card.card-data-wrapper>
                            ...
                            <x-card.card-data-row
                                ...
                                :value="sprintf('%.3f %s', $sku->weight_netto, __('localization.sku_view_net_weight_unit'))"
                            />

                            <x-card.card-data-row
                                :label="__('localization.sku_view_gross_weight')"
                                :value="sprintf('%.3f %s', $sku->weight_brutto, __('localization.sku_view_gross_weight_unit'))"
                            />
                        </x-card.card-data-wrapper>

                        @php
                            $hasAdr = ! empty($sku->adr);
                            $hasTemp = isset($sku->temp_from, $sku->temp_to);
                            $hasHumidity = isset($sku->humidity_from, $sku->humidity_to);
                            $hasDustiness = isset($sku->dustiness_from, $sku->dustiness_to);
                        @endphp

                        @if ($hasAdr || $hasTemp || $hasHumidity || $hasDustiness)
                            <x-ui.section-card-title level="5">
                                {{ __("localization.sku_view_barcode") }}
                            </x-ui.section-card-title>

                            <x-card.card-data-wrapper>
                                @if ($hasAdr)
                                    <x-card.card-data-row
                                        :label="__('localization.sku_view_adr')"
                                        :value="$sku->adr->name"
                                    />
                                @endif
                                ...
                                @if ($hasTemp)
                                    <x-card.card-data-row
                                        :label="__('localization.sku_view_temperature_range')"
                                        :value="$sku->temp_from.' - '.$sku->temp_to.' °C'"
                                    />
                                @endif

                                @if ($hasHumidity)
                                    <x-card.card-data-row
                                        :label="__('localization.sku_view_humidity')"
                                        :value="$sku->humidity_from.' - '.$sku->humidity_to.' %'"
                                    />
                                @endif

                                @if ($hasDustiness)
                                    <x-card.card-data-row
                                        :label="__('localization.sku_view_dustiness')"
                                        :value="__('localization.sku_view_dustiness_range', [
                                            'from' => $sku->dustiness_from,
                                            'to' => $sku->dustiness_to,
                                            'unit' => __('localization.sku_view_dustiness_unit'),
                                        ])"
                                    />
                                @endif
                            </x-card.card-data-wrapper>
...
@section("page-script")
    ...
    <script type="module">
        ...
        let tableData = @json($sku->packages->toArray());
