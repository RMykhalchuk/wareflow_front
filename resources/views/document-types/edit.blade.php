...
@section('title', __('localization.document_types_edit_title'))
...
@section('content')
    ...
    <x-layout.container fluid id="edit-card" data-id="{{$documentType->id}}">
        <x-slot:header>
            @include(
                'panels.breadcrumb',
                [
                    'options' => [
                        [
                            'url' => '/document-type/',
                            'name' => __('localization.document_types_edit_breadcrumb_text_1'),
                        ],
                        [
                            'name' => __('localization.document_types_edit_breadcrumb_text_2'),
                        ],
                    ],
                ]
            )
            <x-ui.header-actions>
                <button type="button" id="draft-edit" class="btn btn-flat-dark">
                    {{ __('localization.document_types_edit_save_draft') }}
                </button>
                <button type="submit" id="doctype-edit" class="btn btn-primary">
                    {{ __('localization.document_types_edit_save') }}
                </button>
            </x-ui.header-actions>
        </x-slot>
            ...
            <x-card.nested>
                <x-slot:header>
                    @php
                        ...
                        $titleKey = "localization.document_types.edit.{$typeKey}";
                    @endphp
                ...
                <x-slot:body>
                                ...
                                >
                                    <div class="mb-1">
                                        <input
                                            ...
                                            placeholder="{{ __('localization.document_types_edit_name_placeholder') }}"
                                            aria-label="{{ __('localization.document_types_edit_name_aria') }}"
                                            aria-describedby="document-type-name"
                                            value="{{ $documentType->name }}"
                                            required
                                        />
                                        <div class="valid-feedback">
                                            {{ __('localization.document_types_edit_name_valid_feedback') }}
                                        </div>
                                        <div class="invalid-feedback">
                                            {{ __('localization.document_types_edit_name_invalid_feedback') }}
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <x-tabs.group class="document-type-tabs" navClass="css-custom-nav-tabs">
                        @slot('items')
                            <x-tabs.item
                                id="header"
                        ...
                        @slot('content')
                                                ...
                                                <div>
                                                    ...
                                                    >
                                                        ...
                                                        @foreach ($fields as $field)
                                                            {{-- Отримуємо значення data-type-block з поля --}}
                                                            @php
                                                                $typeBlock = $field['typeBlock'] ?? 'header'; // Встановлюємо значення за замовчуванням, якщо воно не визначене
                                                            @endphp

                                                            {{-- <script> --}}
                                                            {{-- console.log(@json($field)); // Виводимо значення field в консоль --}}
                                                            {{-- </script> --}}
                                                            @include('document-types.update-fields', ['typeBlock' => $typeBlock])
                                                            {{-- Передати значення data-type-block --}}
                                                        @endforeach
                                                    </ul>
                                                </div>
                                                    ...
                                                    @foreach ($documentType->settings()['custom_blocks'] as $k => $block)
                                                        <div class="new-fields-custom-block">
                                                            ...
                                                            >
                                                                ...
                                                                <button
                                                                    class="btn btn-flat-secondary p-25 d-none"
                                                            ...
                                                            <ul
                                                                class="sortableList sortableListStyle ui-sortable"
                                                                data-type-block="custom_block{{ $k }}"
                                                                id="custom_block-{{ $k }}_fields"
                                                            >
                                                                @foreach ($fieldsCustomBlock as $key => $field)
                                                                    @php
                                                                        $typeBlock = $field['typeBlock'] ?? "custom_block_{$k}"; // Встановлюємо значення за замовчуванням, якщо воно не визначене
                                                                    @endphp
                            ...
                            <x-tabs.content id="documents" :active="false">
                                <div class="document-fields px-0">
                                    <div class="row mx-0">
                                        ...
                                        >
                                            @php
                                                $documents = $documentType->settings()['document_type'] ?? [];
                                            @endphp
                                            ...
                                            >
                                                ...
                                                <li
                                                    ...
                                                    class="text-center p-2 text-muted px-5 border-dashed fw-bold bg-secondary-100 rounded {{ count($documents) ? 'd-none' : '' }}"
                                                >
                                                    {{ __('localization.document_types.documents.fields-list.placeholder') }}
                                                </li>

                                                @foreach ($documents as $docKey => $docItem)
                                                    <x-pages.doc-types.document-item
                                                        :id="$docKey"
                                                        :name="$docItem['name'] ?? $docKey"
                                                        :draggable="true"
                                                    />
                                                @endforeach
                                            </ul>
                                ...
                                <div class="document-fields px-0">
                                    <div class="row mx-0">
                                        ...
                                        >
                                            <div>
                                                @php
                                                    $actions = $documentType->settings()['actions'];
                                                @endphp
                            ...
                            <x-tabs.content id="task" :active="false">
                                @php
                                    $tasks = $documentType->settings()['tasks'] ?? [];
                                @endphp
