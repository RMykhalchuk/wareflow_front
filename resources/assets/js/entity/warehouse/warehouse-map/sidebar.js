const LEVEL_LABELS = { zones: 'Зони', sectors: 'Сектори', rows: 'Ряди' };
const LEVEL_ICONS = { zones: 'bi-grid', sectors: 'bi-geo-alt', rows: 'bi-list-ul' };
...
export function createSidebar({ onDragStart, onItemClick }) {
    ...
    function makeItemRow(item, isPlaced) {
        ...
        name.style.cssText =
            'font-size:13px;font-weight:500;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap';
        if (isPlaced) name.style.color = '#6c757d';
        name.textContent = item.name;

        const icon = document.createElement('i');
        icon.className = isPlaced
            ? 'bi bi-check-circle-fill text-success'
            : 'bi bi-plus text-secondary';
