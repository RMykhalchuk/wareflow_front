...
export async function initWarehouseMap(container) {
    ...
    loadingEl.innerHTML =
        '<div class="spinner-border text-primary" role="status"><span class="visually-hidden">Завантаження...</span></div>';
    ...
    const grid = createGrid({
        onDrop(elementId, gridX, gridY) {
            const s = store.getState();
            const allItems = [...s.zones, ...s.sectors, ...s.rows];
            const item = allItems.find((i) => i.id === elementId);
        ...
        onUpdate(id, updates) {
            ...
            store.setState((prev) => ({
                ...prev,
                mapElements: prev.mapElements.map((el) =>
        ...
        onNavigate(elementId) {
            ...
            if (s.nav.level === 'zones') {
                ...
                store.setState((prev) => ({
                    ...prev,
                    nav: { level: 'sectors', zoneId: elementId, zoneName: zone?.name },
                }));
    ...
    async function placeItem(s, item, gridX, gridY) {
        ...
        const elementType =
            nav.level === 'zones' ? 'zone' : nav.level === 'sectors' ? 'sector' : 'row';
        const parentId =
            nav.level === 'sectors' ? nav.zoneId : nav.level === 'rows' ? nav.sectorId : null;
    ...
    function getLabel(s, elementId) {
        if (s.nav.level === 'zones')
            return s.zones.find((z) => z.id === elementId)?.name ?? elementId;
        if (s.nav.level === 'sectors')
            return s.sectors.find((sc) => sc.id === elementId)?.name ?? elementId;
        return s.rows.find((r) => r.id === elementId)?.name ?? elementId;
    }

    function getChildCount(s, elementId) {
        if (s.nav.level === 'zones')
            return s.sectors.filter((sc) => sc.zone_id === elementId).length;
        if (s.nav.level === 'sectors')
            return s.rows.filter((r) => r.sector_id === elementId).length;
        return s.shelves.filter((sh) => sh.row_id === elementId).length;
    }

    function getChildLabel(s, count) {
        if (s.nav.level === 'zones')
            return count === 1 ? 'сектор' : count >= 2 && count <= 4 ? 'сектори' : 'секторів';
        if (s.nav.level === 'sectors')
            return count === 1 ? 'ряд' : count >= 2 && count <= 4 ? 'ряди' : 'рядів';
    ...
    function getBreadcrumbItems(s) {
        ...
        const items = [
            {
                label: 'Склад',
                onClick: () => store.setState((prev) => ({ ...prev, nav: { level: 'zones' } })),
            },
        ];
        if (nav.level === 'sectors' || nav.level === 'rows' || nav.level === 'rowmap') {
            items.push({
                ...
                onClick: () =>
                    store.setState((prev) => ({
                        ...prev,
                        nav: { level: 'sectors', zoneId: nav.zoneId, zoneName: nav.zoneName },
                    })),
            });
        }
    ...
    store.subscribe((s) => {
            ...
            rowMapInstance = createRowMap({
                ...
                onBack: () =>
                    store.setState((prev) => ({
                        ...
                        nav: {
                            level: 'rows',
                            zoneId: prev.nav.zoneId,
                            zoneName: prev.nav.zoneName,
                            sectorId: prev.nav.sectorId,
                            sectorName: prev.nav.sectorName,
                        },
                    })),
            });
        ...
        const elementType =
            s.nav.level === 'zones' ? 'zone' : s.nav.level === 'sectors' ? 'sector' : 'row';
