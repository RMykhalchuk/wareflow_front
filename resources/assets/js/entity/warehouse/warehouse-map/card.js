...
export function createCard({
    ...
    function openPicker() {
        ...
        colorPickerInstance = createColorPicker(currentElement.color, (color) => {
            currentElement = { ...currentElement, color };
            applyPosition();
            closePicker();
            onUpdate({ color });
        });
    ...
    function render() {
        el.innerHTML = '';
        el.className = `wh-card map-element-card${isDragging ? ' dragging' : ''}${isResizing ? ' resizing' : ''}${showColorPicker ? ' picker-open' : ''}`;
        applyPosition();
        ...
        if (readonly) {
            ...
            if (currentChildCount !== undefined && currentChildCount !== null) {
                ...
                countBadge.textContent = currentChildLabel
                    ? `${currentChildCount} ${currentChildLabel}`
                    : currentChildCount;
        ...
        if (showColorPicker && colorPickerWrap) {
            el.appendChild(colorPickerWrap);
        }
    }
    ...
    document.addEventListener('click', (e) => {
        if (
            showColorPicker &&
            colorPickerWrap &&
            !colorPickerWrap.contains(e.target) &&
            e.target !== paletteBtn
        ) {
            closePicker();
        }
