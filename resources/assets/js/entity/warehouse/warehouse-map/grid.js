...
export function createGrid({ onDrop, onUpdate, onDelete, onNavigate, canNavigate, readonly }) {
    ...
    function applyGridSize() {
        ...
        cardInstances.forEach(
            (inst) => inst.updateGridBounds && inst.updateGridBounds(gridCols, gridRows)
        );
    }
            ...
            function onMove(me) {
                ...
                if (direction === 'right' || direction === 'both') {
                    const newCols = Math.max(
                        MIN_COLS,
                        Math.min(MAX_COLS, Math.round(startCols + dx / CELL_SIZE))
                    );
                    gridCols = newCols;
                }
                if (direction === 'bottom' || direction === 'both') {
                    const newRows = Math.max(
                        MIN_ROWS,
                        Math.min(MAX_ROWS, Math.round(startRows + dy / CELL_SIZE))
                    );
                    gridRows = newRows;
                }
                ...
                showTooltip(rect.right - wrapperRect.left - 80, rect.bottom - wrapperRect.top - 36);
            }
        ...
        items.forEach(({ mapElement, label, childCount, childLabel }) => {
            if (cardInstances.has(mapElement.id)) {
                ...
                const card = createCard({
                    ...
                    onUpdate: (updates) => {
                        const current =
                            currentItems.find((i) => i.mapElement.id === mapElement.id)
