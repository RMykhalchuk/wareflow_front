...
$(document).ready(function () {
    ...
    let dataFields = [
        ...
        { name: 'task_type_name', map: 'type>name', type: 'string' },
    ...
    let myDataSource = isTestMode
        ...
        : {
              ...
              url:
                  ...
                  languageBlock +
                  `/document/${documentID}/task/table/filter`,
              root: 'data',
    ...
    var grid = table.jqxGrid({
        ...
        rendertoolbar: function (statusbar) {
            ...
            return toolbarRender(statusbar, table, false, 1, columnCount - 1, '-tasks');
        },
        ...
        columns: [
            ...
            // },
            {
                ...
                cellsrenderer: (row, column, value, defaulthtml, columnproperties, rowdata) => {
                    return `<p class="ps-50 my-auto">${rowdata.task_type_name}</p>`;
                },
            },
            {
            ...
            },
            {
                ...
                width: 200,
                cellsrenderer: (row, column, value, defaulthtml, columnproperties, rowdata) => {
                    let displayValue = 'Усі';

                    if (Array.isArray(value) && value.length > 0) {
                        // Якщо масив об'єктів (наприклад [{name: 'Іван'}, {name: 'Олег'}])
                        // або просто масив рядків (['Іван', 'Олег'])
                        displayValue = value
                    ...
                    return `
                                <p class="ps-50 text-dark fw-bolder my-auto">
                                    ${displayValue || 'Усі'}
                                </p>
                            `;
                },
            },
            {
                ...
                cellsrenderer: (row, column, value, defaulthtml, columnproperties, rowdata) => {
                    ...
                    // Форматуємо у зрозумалий вигляд
                    const created_date = date.toLocaleDateString('uk-UA', {
                        year: 'numeric',
                        month: '2-digit',
                        day: '2-digit',
                    });

                    const created_time = date.toLocaleTimeString('uk-UA', {
                        hour: '2-digit',
                        minute: '2-digit',
                    });
    ...
    listbox(table, listSource, '-tasks');
...
const statusMap = {
    planned: {
        class: 'bg-secondary',
        text: getLocalizedText('status_planned'), // Запланована
    },
    rescan: {
        class: 'bg-primary',
        text: getLocalizedText('status_rescan'), // Перерахунок товару
    },
    inventoried: {
        class: 'bg-info',
        text: getLocalizedText('status_inventoried'), // Товар проінвенторизовано
    },
    erp_check: {
        class: 'bg-warning',
        text: getLocalizedText('status_erp_check'), // Звірка з ERP (опційно)
    },
    completed: {
        class: 'bg-dark',
        text: getLocalizedText('status_completed'), // Завершена / достроково завершена
    },
    paused: {
        class: 'bg-danger',
        text: getLocalizedText('status_paused'), // Пауза
    },
};
