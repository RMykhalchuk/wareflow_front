...
(function (window, document, $) {
    ...
    if ($body.attr('data-framework') !== 'laravel') {
        ...
        $('.dropdown-language .dropdown-item').on('click', function () {
            ...
            // контейнер конкретного dropdown
            var $dropdown = $this.closest('.dropdown-language');
            ...
            // клас прапора (обидва флаг-класи)
            var selectedFlag = $this.find('.flag-icon').attr('class');

            // оновлюємо текст та прапор тільки всередині цього dropdown
            $dropdown.find('.selected-language').text(selectedLang);
            $dropdown.find('.flag-icon').attr('class', selectedFlag);

            // отримуємо код мови
            var currentLanguage = $this.data('language');

            // оновлюємо data-i18n
            var $selectedText = $dropdown.find('.selected-language');
            if (currentLanguage === 'uk') {
                $selectedText.attr('data-i18n', 'LangTitleUA');
            } else if (currentLanguage === 'en') {
                $selectedText.attr('data-i18n', 'LangTitleEN');
            } else if (currentLanguage === 'de') {
                $selectedText.attr('data-i18n', 'LangTitleGE');
            } else if (currentLanguage === 'pl') {
                $selectedText.attr('data-i18n', 'LangTitlePL');
            }
            ...
            if (currentLanguage === 'uk') {
                result = pathname.includes('/uk')
                    ? origin + pathname + search
                    : origin + '/uk' + pathname + search;
            } else {
                result = origin + pathname.replace('/uk', '') + search;
            }
