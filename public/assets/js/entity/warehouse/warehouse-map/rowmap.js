...
export function createRowMap({ rowId, rowName, shelves, cells, onBack }) {
        ...
        rowShelves.forEach((shelf) => {
            ...
            floors.forEach((f) => {
                ...
                shelfCells.forEach((cell) => {
                    ...
                    btn.type = 'button';
                    btn.className =
                        'rm-cell-btn' + (selectedCell?.id === cell.id ? ' rm-cell-selected' : '');
    ...
    function renderContentsPanel(cell) {
        ...
        const pageItems = contents.slice(
            (contentsPage - 1) * CONTENTS_PER_PAGE,
            contentsPage * CONTENTS_PER_PAGE
        );
