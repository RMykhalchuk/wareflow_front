                    ...
                    rand < 0.55
                        ...
                        : rand < 0.7
                          ...
                          : rand < 0.85
                            ...
                            : 'blocked';
                const cellType =
                    Math.random() < 0.8 ? 'standard' : Math.random() < 0.5 ? 'heavy' : 'cold';
                const maxW = cellType === 'heavy' ? 1000 : 500;
                const curW =
                    status === 'occupied'
                        ? Math.floor(Math.random() * maxW * 0.8)
                        : status === 'reserved'
                          ? Math.floor(Math.random() * maxW * 0.3)
                          : 0;
                const maxVol = 200;
                const curVol =
                    status === 'occupied'
                        ? Math.floor(Math.random() * maxVol * 0.85)
                        : status === 'reserved'
                          ? Math.floor(Math.random() * maxVol * 0.3)
                          : 0;
                cells.push({
                    id: `cell-${rowId}-${s}-${f}-${p}`,
                    shelf_id: shelfId,
...
function generateContents() {
    ...
    return Array.from({ length: count }, (_, i) => ({
        ...
        name:
            ...
            ` код ${1000 + Math.floor(Math.random() * 9000)} м2`,
