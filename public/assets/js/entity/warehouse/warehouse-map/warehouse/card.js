...
export function createCard({ element, label, gridCols, gridRows, onUpdate, onDelete, onNavigate, canNavigate, readonly: initialReadonly }) {
  ...
  function applyPosition() {
    ...
    el.style.backgroundColor = currentElement.color + '33';
  ...
  function openPicker() {
    ...
    paletteBtn.parentNode.appendChild(colorPickerWrap);
  }
  ...
  function render() {
    ...
    header.style.backgroundColor = currentElement.color + '66';
    ...
    if (!readonly) {
      ...
      grip.style.cssText = 'font-size:11px;color:#6c757d;flex-shrink:0';
    ...
    if (readonly) {
      ...
      lbl.style.cssText = `font-size:13px;font-weight:600;color:${currentElement.color};display:block;text-align:center`;
      lbl.textContent = currentLabel;
      body.appendChild(lbl);
      if (canNavigate) {
        const hint = document.createElement('span');
        hint.style.cssText = 'font-size:10px;color:#adb5bd;display:flex;align-items:center;justify-content:center;gap:2px;margin-top:2px';
        hint.innerHTML = 'натисніть для переходу <i class="bi bi-chevron-right" style="font-size:9px"></i>';
        body.appendChild(hint);
      }
    } else {
      const sizeHint = document.createElement('span');
      sizeHint.style.cssText = `font-size:11px;color:${currentElement.color};opacity:0.6`;
  ...
  function update(newElement, newLabel) {
