...
import { initLeftoversGrid } from './leftovers-table.js';
import { refreshProgress } from '../../../entity/document/outcome/document-leftovers.js';
...
$(document).ready(function () {
    ...
    const documentStatus = $('#document-container').data('status');
    const isCompleted = documentStatus === 'done';
    ...
    var grid = table.jqxGrid({
        ...
        rendertoolbar: function (statusbar) {
            ...
            return toolbarRender(statusbar, table, true, 1, columnCount - 1, '-sku'); // Subtract 1 to exclude the action column
        },
        ...
        columns: [
            ...
            },
            {
                ...
                cellsrenderer: (row, column, value, defaulthtml, columnproperties, rowdata) => {
                    ...
                    const actionButton =
                        activity_state === 'manual' && !isCompleted
                            ? `
                            <div type="button"
                                ...
                                ${getLocalizedText('tablePreviewSkuLeftoversAction')}
                            </div>`
                            : '';
    ...
    $(document).on('click', '.leftovers-btn', function () {
        ...
        // 🔹 Записуємо id для кнопки підтвердження
        $('#add_leftovers_submit').data('sku-unique-id', skuUniqueId);

        // 🔹 Оновлюємо Select2
        const $goodsSelect = $('#goods_id');
        const newOption = new Option(cellName, uuid, true, true);
        ...
        if (!gridInitialized) {
            initLeftoversGrid(skuUniqueId, uuid, unit);
            ...
            if (dataAdapter && dataAdapter._source) {
                // додаємо SKU як параметр для запиту на сервер
                dataAdapter._source.url = `${location.origin + languageBlock}/document/outcome/leftover/${encodeURIComponent(documentID)}/${encodeURIComponent(skuUniqueId)}/table/filter`;
