        ...
        columns: [
            ...
            },
            {
                ...
                cellsrenderer: (row, column, value, defaulthtml, columnproperties, rowdata) => {
                    ...
                    // показуємо кнопку тільки якщо manual І документ НЕ завершений
                    const buttonHtml =
    ...
    $(document).on('click', '.leftovers-btn', function () {
        ...
        if (!gridInitialized) {
            ...
            if (dataAdapter && dataAdapter._source) {
                ...
                dataAdapter._source.url = `${location.origin + languageBlock}/document/income/leftover/${encodeURIComponent(documentID)}/${encodeURIComponent(skuUniqueId)}/table/filter`;
