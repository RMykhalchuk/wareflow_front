...
final class FormatTableData extends AbstractFormatTableData
{
    ...
    public function formatData($leftovers)
    {
        ...
        foreach ($leftovers as $key => $leftover) {
            $leftoversArray[] = [
                ...
                'goods' => [
                    ...
                    'manufacturer' => $leftover->goods?->manufacturerCompany?->full_name ?: $leftover->goods->manufacturer,
                    'category' => $leftover->goods?->category?->name,
                    'provider' => $leftover->goods?->providerCompany?->full_name ?: $leftover->goods->provider
                ],
                'status' => $leftover->status,
                'batch' => $leftover->batch,
                'manufacture_date' => $leftover->manufacture_date?->toDateString(),
                'bb_date'          => $leftover->bb_date?->toDateString(),
                'package' => $leftover->package->name,
                'has_condition' => $leftover->has_condition,
                'allocation' => (function () use ($leftover) {
                    $allocation = isset($leftover->cell)
                        ? ($leftover->cell->allocation ?? [])
                        : ($leftover->container?->cell?->allocation ?? []);

                    if ($leftover->container) {
                        $allocation['container'] = $leftover->container->code;
                    }

                    return $allocation;
                })(),
                'quantity' => [
                    'measurement_unit_quantity' => round($leftover->quantity * $leftover->package->main_units_number, 3),
        ...
        return TableCollectionResource::make($leftoversArray)->setTotal($leftovers->total());
    }
